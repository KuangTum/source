...
##kestrel OpenMPI
# CC = mpicc -O3 -ffast-math -fcommon -fopenmp -I$(FFTW_ROOT_DIR)/include -I$(OPENMPI_ROOT_DIR)/include 
# FC = mpif90 -O3 -ffast-math -fallow-argument-mismatch -fopenmp -I$(FFTW_ROOT_DIR)/include -I$(OPENMPI_ROOT_DIR)/include
# LIB= -L$(FFTW_ROOT_DIR)/lib -lfftw3 -L$(OPENMPI_ROOT_DIR)/lib -lmpi -lmpi_mpifh -L$(NETLIB_SCALAPACK_ROOT_DIR)/lib -lscalapack -L$(NETLIB_LAPACK_ROOT_DIR)/lib64 -L$(NETLIB_LAPACK_ROOT_DIR)/lib64 -llapack -lblas -lgfortran 

##kesrel intelmpi with -DOLPR_DUMP to dump the overlap matrix and position matrix
CC = mpiicc -O3 -xHOST -ip -no-prec-div -qopenmp -DOLPR_DUMP -I${MKLROOT}/include/fftw -I${MKLROOT}/include
