...
function parse_openmx_overlaponly(filepath::String)
    ...
    # 只读 S 与 r（第一阶）；OLP 可能带自旋通道
    OLP = read_matrix_in_mixed_matrix(Float64, f, SpinP_switch + 1, atomnum, FNAN, natn, TNO)
    ...
    for axis in 1:3, i in 1:atomnum, h in 1:FNAN[i]
        OLP_r[axis][i][h] .+= Gxyz[axis,i] * OLP[1][i][h]
    end
...
function createmodelopenmx_overlaponly(filepath::String)
    ...
    # 仅由 Dump_OverlapOnly_SCFOUT 写出；不包含 Hk/iHk 等块
    OLP = read_matrix_in_mixed_matrix(Float64, f, SpinP_switch + 1, atomnum, FNAN, natn, Total_NumOrbs)
    OLP_r = Vector{Vector{Vector{Matrix{Float64}}}}(undef, 3)
    for dir in 1:3
        # order_max 被 dump 端固定为 1
        OLP_r[dir] = read_matrix_in_mixed_matrix(Float64, f, 1, atomnum, FNAN, natn, Total_NumOrbs)[1]
    end
    ...
    for axis in 1:3, i in 1:atomnum, j in 1:FNAN[i]
        OLP_r[axis][i][j] .+= Gxyz[axis, i] * OLP[1][i][j]
    end
    ...
    if SpinP_switch == 0
        ...
        for i in 1:atomnum, j in 1:FNAN[i], ii in 1:Total_NumOrbs[i], jj in 1:Total_NumOrbs[natn[i][j]]
            setoverlap!(nm, atv_ijk[:, ncn[i][j]], numorb_base[i] + ii, numorb_base[natn[i][j]] + jj, OLP[1][i][j][jj, ii])
            for alpha in 1:3
                setposition!(nm, atv_ijk[:, ncn[i][j]], numorb_base[i] + ii, numorb_base[natn[i][j]] + jj, alpha, OLP_r[alpha][i][j][jj, ii])
            end
        end
    elseif SpinP_switch == 1
        nm = TBModel{Float64}(norb * 2, tv, isorthogonal=false)
        for sp in 0:1, i in 1:atomnum, j in 1:FNAN[i], ii in 1:Total_NumOrbs[i], jj in 1:Total_NumOrbs[natn[i][j]]
            setoverlap!(nm, atv_ijk[:, ncn[i][j]],
                        sp * norb + numorb_base[i] + ii,
                        sp * norb + numorb_base[natn[i][j]] + jj,
                        OLP[sp + 1][i][j][jj, ii])
            for alpha in 1:3
                setposition!(nm, atv_ijk[:, ncn[i][j]],
                            sp * norb + numorb_base[i] + ii,
    ...
    elseif SpinP_switch == 3
        nm = TBModel{ComplexF64}(norb * 2, tv, isorthogonal=false)
        for spini in 0:1, spinj in 0:1, i in 1:atomnum, j in 1:FNAN[i],
            ii in 1:Total_NumOrbs[i], jj in 1:Total_NumOrbs[natn[i][j]]
            if spini == spinj
                block = spini == 0 ? 1 : 2
                setoverlap!(nm, atv_ijk[:, ncn[i][j]],
                            spini * norb + numorb_base[i] + ii,
                            spinj * norb + numorb_base[natn[i][j]] + jj,
                            OLP[block][i][j][jj, ii])
            end
        end
        for spini in 0:1, spinj in spini:1, i in 1:atomnum, j in 1:FNAN[i],
            ii in 1:Total_NumOrbs[i], jj in 1:Total_NumOrbs[natn[i][j]], alpha in 1:3
            setposition!(nm, atv_ijk[:, ncn[i][j]],
                        spini * norb + numorb_base[i] + ii,
                        spinj * norb + numorb_base[natn[i][j]] + jj,
                        alpha, OLP_r[alpha][i][j][jj, ii])
